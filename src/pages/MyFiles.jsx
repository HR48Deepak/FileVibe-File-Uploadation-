...
import AddFileDrawer from "../components/AddFileDrawer";
...
const MyFiles = () => {
    ...
    const confirmDelete = async () => {
        ...
        if (response.ok) {
            setFiles(file.filter(f => f._id !== id));
    ...
    const confirmShare = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // alert(`Error: ${errorData.message || 'Not shared'}`);
    ...
    return (
        <>
            ...
            <div className='min-h-screen bg-slate-600 w-full flex overflow-x-hidden relative font-sans'>
                ...
                {menuVisible && (
                    <div className="fixed inset-0 bg-black/50 z-40 lg:hidden" onClick={() => setMenuVisible(false)} />
                )}
                <div className={`bg-slate-400 min-h-screen transition-all duration-300 ease-in-out z-50 ${menuVisible ? "w-80 translate-x-0 relative" : "w-0 -translate-x-full absolute"}`}>
                    <div className={`w-80 ${menuVisible ? "opacity-100" : "opacity-0 pointer-events-none"} transition-opacity duration-200`}>
                        <div className="flex justify-end p-4 lg:hidden">
                            <FaTimes className="text-white text-2xl cursor-pointer" onClick={() => setMenuVisible(false)} />
                        </div>
                        ...
                        <div className='flex flex-col gap-6 px-6 items-center mt-7'>
                            ...
                            <Link to="/my-files" className="text-white flex bg-slate-600 gap-2 flex-row border-2 rounded-lg p-2 cursor-pointer w-32.5 hover:scale-105">
                ...
                <div className="flex flex-col flex-1 min-w-0">
                    <div className="bg-slate-200 px-6 py-4 w-full shadow-lg flex justify-between items-center h-20">
                        ...
                        <div className="flex gap-3">
                            ...
                            <button onClick={() => setMenuVisible(!menuVisible)} className="bg-slate-300 p-3 rounded hover:bg-slate-400 transition-colors">
                                <HiMenuAlt1 />
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="p-6 space-y-4">
                        <div className="flex justify-between items-center">
                            <h2 className="text-3xl font-bold text-white mb-8">My Files</h2>
                            <button onClick={() => setIsDrawerOpen(true)} className="bg-white px-4 py-2 rounded-lg font-bold hover:scale-105 transition-transform">
                                + Add Files
                            </button>
                        </div>
                                    ...
                                    {currentFiles.length > 0 ? (
                                        currentFiles.map((f) => (
                                            <tr key={f._id} className="border-b hover:bg-slate-50 truncate transition-colors divide-x overflow-x-auto">
                                                <td className="px-4 py-4 ">{f.filename}</td>
                                                <td className="px-4 py-4 ">{f.type}</td>
                                                <td className="px-4 py-4 ">{(f.size / 1024).toFixed(2)} KB</td>
                                                <td className="px-4 py-4 ">{new Date(f.createdAt).toLocaleDateString()}</td>
                                                <td className="py-4 px-6">
                                                    <div className="flex gap-4">
                                                        <button onClick={() => setDeleteModal({ isOpen: true, id: f._id, publicId: f.public_id })} className="text-red-500 hover:text-red-700" title="Delete">
                                                            <MdDeleteForever size={22} />
                                                        </button>
                                                        <button onClick={() => setShareModal({ isOpen: true, file: f, email: '' })} className="text-green-600 hover:text-green-800" title="Share">
                                                            <FaShare size={20} />
                                                        </button>
                                                        <button onClick={() => handleDownload(f._id, f.filename)} className="text-blue-600 hover:text-blue-800" title="Download">
                    ...
                    <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                            ...
                            <div className="flex justify-end gap-3">
                                <button onClick={() => setShareModal({ isOpen: false, file: null, email: '' })} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg">Cancel</button>
                                <button onClick={confirmShare} disabled={loading} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">
                ...
                {deleteModal.isOpen && (
                    <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
                            ...
                            <div className="flex gap-3">
                                <button onClick={() => setDeleteModal({ isOpen: false, id: null, publicId: null })} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">Cancel</button>
                                <button onClick={confirmDelete} className="flex-1 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-semibold">Delete</button>
                            </div>
                        </div>
                    </div>
                )}

                <AddFileDrawer isOpen={isDrawerOpen} onClose={() => setIsDrawerOpen(false)} onUploadSuccess={fetchFiles} />
            </div>
        </>
    );
};
